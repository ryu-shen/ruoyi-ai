...
public class GlobalExceptionHandler {
	...
	public R<Void> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e,
		...
		return R.fail(String.format("请求参数类型不匹配，参数[%s]要求类型为：'%s'，但输入值为：'%s'", e.getName(), e.getRequiredType().getName(),
				e.getValue()));
	}
	 ...
	 * Ryu新增，拦截自定义运行时异常
	 */
	@ExceptionHandler(AgiServiceException.class)
	public Result<Void> handleCustomRuntimeException(RuntimeException e, HttpServletRequest request) {
		String requestURI = request.getRequestURI();
		log.error("请求地址'{}',发生未知异常.", requestURI, e);
