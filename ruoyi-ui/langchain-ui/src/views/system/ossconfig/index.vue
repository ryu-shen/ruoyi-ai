<script lang="ts" setup>
  ...
  import { del, page as getPage } from '@/api/system/ossconfig';
  ...
  function handleEdit(record: Recordable) {
    editRef.value.show(record.ossConfigId);
  }

  function handleDelete(record: Recordable) {
    dialog.info({
      title: '提示',
      content: `您想删除 ${record.configKey}`,
      positiveText: '确定',
      negativeText: '取消',
      onPositiveClick: async () => {
        await del(record.ossConfigId);
  ...
  <div class="h-full">
    <n-card :bordered="false">
      ...
      <BasicTable
        ...
        :row-key="(row:any) => row.ossConfigId"
        :single-line="false"
        :size="'small'"
      >
        <template #tableTitle>
